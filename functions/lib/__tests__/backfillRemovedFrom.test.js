...
async function callBackfill(data = {}, context = makeContext()) {
...
describe('Cenário 1 — caller não-admin', () => {
    it('lança permission-denied quando email não está em /admins/', async () => {
        await expect(callBackfill({}, makeContext('uid-rand', NON_ADMIN_EMAIL))).rejects.toMatchObject({ code: 'permission-denied' });
        expect(commitCount).toBe(0);
        expect(batchCalls).toHaveLength(0);
    });
    it('lança permission-denied quando token não tem email', async () => {
        await expect(callBackfill({}, { auth: { uid: 'x', token: {} } })).rejects.toMatchObject({ code: 'permission-denied' });
    });
});
// ── Cenário 2: sem auth ──────────────────────────────────────────────────────
describe('Cenário 2 — sem context.auth', () => {
    it('lança unauthenticated', async () => {
        await expect(callBackfill({}, { auth: null })).rejects.toMatchObject({ code: 'unauthenticated' });
        expect(commitCount).toBe(0);
    });
});
...
describe('Cenário 3 — admin SaaS processa docs com uid', () => {
    it('emite arrayUnion(schoolId) em users/{uid} para cada doc com uid', async () => {
        ...
        expect(setAaa.data.removedFrom).toMatchObject({ __op__: 'arrayUnion', values: ['sch-1'] });
        expect(setBbb.data.removedFrom).toMatchObject({ __op__: 'arrayUnion', values: ['sch-2'] });
        expect(setCcc.data.removedFrom).toMatchObject({ __op__: 'arrayUnion', values: ['sch-1'] });
...
describe('Cenário 5 — arrayUnion garante idempotência', () => {
    it('grava removedFrom como wrapper arrayUnion (não array literal)', async () => {
        ...
        const data = setX.data;
        expect(Array.isArray(data.removedFrom)).toBe(false);
        expect(data.removedFrom.__op__).toBe('arrayUnion');
    });
...
describe('Cenário 7 — tolerância a falhas de commit', () => {
    it('incrementa errors quando um commit intermediário falha', async () => {
        ...
        expect(result.processed).toBe(850);
    });
});
...
//# sourceMappingURL=backfillRemovedFrom.test.js.map