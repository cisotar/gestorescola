...
function getRemoveTeacherHandler() {
...
async function callRemoveTeacher(data, context = makeContext()) {
...
describe('Cenário 1 — users/{uid} existe', () => {
    ...
    it('mantém a criação de schools/{schoolId}/removed_users/{uid}', async () => {
        ...
        const removedUser = findCall('set', `schools/${SCHOOL_ID}/removed_users/${TEACHER_UID}`);
...
describe('Cenário 3 — idempotência via arrayUnion', () => {
    ...
    it('chamadas repetidas geram sentinels arrayUnion equivalentes (no servidor o set é idempotente)', async () => {
        ...
        expect(arrayUnionMock.mock.calls.every((args) => args[0] === SCHOOL_ID)).toBe(true);
    });
});
...
describe('Cenário 4 — auto-remoção do admin', () => {
    it('lança failed-precondition quando callerUid === teacherUid', async () => {
        ...
        await expect(callRemoveTeacher({
            schoolId: SCHOOL_ID,
            teacherId: TEACHER_DOC_ID,
        }, makeContext(TEACHER_UID, TEACHER_EMAIL))).rejects.toMatchObject({
...
//# sourceMappingURL=removeTeacherFromSchool.test.js.map