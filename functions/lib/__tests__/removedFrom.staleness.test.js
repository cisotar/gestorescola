...
function getHandler(idx) {
    ...
    if (!handler)
        throw new Error(`Handler ${idx} não capturado`);
    return handler;
}
function makeContext(uid = CALLER_UID, email = CALLER_EMAIL) {
    return {
        auth: {
            uid,
            token: { email },
...
describe('joinSchoolAsAdmin — limpeza de removedFrom (ALTA #1)', () => {
    it('inclui removedFrom: arrayRemove(schoolId) no set+merge de users/{callerUid}', async () => {
        await getHandler(JOIN_AS_ADMIN_INDEX)({ schoolId: SCHOOL_ID }, makeContext(CALLER_UID, CALLER_EMAIL));
...
describe('setTeacherRoleInSchool — limpeza de removedFrom (ALTA #2)', () => {
    ...
    it('quando users/{teacherUid} existe SEM entry para schoolId, batch.set inclui removedFrom: arrayRemove(schoolId)', async () => {
        ...
        expect(data.schools[SCHOOL_ID]).toMatchObject({
            role: 'admin',
            status: 'approved',
...
//# sourceMappingURL=removedFrom.staleness.test.js.map