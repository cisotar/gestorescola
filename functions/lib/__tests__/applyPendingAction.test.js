...
async function callApplyPendingAction(data, context = makeContext()) {
...
describe('validação de pendingActionId', () => {
    it('lança invalid-argument quando pendingActionId está ausente', async () => {
        await expect(callApplyPendingAction({ approved: true })).rejects.toMatchObject({
            code: 'invalid-argument',
            message: expect.stringContaining('pendingActionId'),
        });
    });
    it('lança invalid-argument quando pendingActionId é string vazia', async () => {
        await expect(callApplyPendingAction({ pendingActionId: '', approved: true })).rejects.toMatchObject({ code: 'invalid-argument' });
    });
...
describe('pendingDoc não encontrado', () => {
    ...
    it('lança not-found quando pendingDoc.exists é false', async () => {
        await expect(callApplyPendingAction({ pendingActionId: PENDING_ACTION_ID, approved: true })).rejects.toMatchObject({ code: 'not-found' });
    });
...
describe('idempotência — status já "approved"', () => {
    ...
    it('lança failed-precondition quando status já é "approved"', async () => {
        await expect(callApplyPendingAction({ pendingActionId: PENDING_ACTION_ID, approved: true })).rejects.toMatchObject({
...
describe('idempotência — status já "rejected"', () => {
    ...
    it('lança failed-precondition quando status já é "rejected"', async () => {
        await expect(callApplyPendingAction({ pendingActionId: PENDING_ACTION_ID, approved: false })).rejects.toMatchObject({
            code: 'failed-precondition',
            message: expect.stringContaining('processada'),
        });
    });
...
describe('audit log em admin_actions', () => {
    ...
    it('grava actorEmail em lowercase', async () => {
        await callApplyPendingAction({ pendingActionId: PENDING_ACTION_ID, approved: true }, makeContext('Admin@Escola.Example.COM', ACTOR_UID));
        const setArg = mockDocSet.mock.calls[0][0];
...
describe('caminhos multi-tenant com schoolId', () => {
    ...
    it('acessa schools/{schoolId}/pending_actions para o update de status', async () => {
        ...
        const pendingActionsPaths = capturedCollectionPaths.filter((p) => p === `schools/${SCHOOL_ID}/pending_actions`);
...
describe('propagação de erro de autorização', () => {
    it('propaga erro de verifyAdmin sem executar nenhuma operação', async () => {
        ...
        auth.verifyAdmin.mockRejectedValueOnce(new HttpsError('permission-denied', 'Não autorizado'));
        await expect(callApplyPendingAction({
            pendingActionId: PENDING_ACTION_ID, approved: true,
...
//# sourceMappingURL=applyPendingAction.test.js.map