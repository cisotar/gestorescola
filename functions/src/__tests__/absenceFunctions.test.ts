...
let mockTeacherQueryEmpty: boolean

// updateAbsence: controla se o absence doc existe
let mockAbsenceDocExists: boolean
...
function getHandler(index: number): Handler {
  const handler = registeredHandlers[index]
  if (!handler) throw new Error(`Handler no índice ${index} não capturado`)
  return handler
}
...
async function callCreate(data: Record<string, unknown>) {
  return getHandler(CREATE_ABSENCE_INDEX)(data, makeContext())
}

async function callUpdate(data: Record<string, unknown>) {
  return getHandler(UPDATE_ABSENCE_INDEX)(data, makeContext())
}

async function callDelete(data: Record<string, unknown>) {
...
beforeEach(() => {
  ...
  // Restaurar verifyCoordinatorOrAdmin para sucesso por padrão
  ;(jest.requireMock('../auth') as { verifyCoordinatorOrAdmin: jest.Mock })
...
describe('createAbsence', () => {
  ...
  describe('professor não encontrado', () => {
    ...
    it('lança not-found quando doc não existe nem por query', async () => {
      await expect(
        callCreate({ teacherId: TEACHER_ID, slots: [] })
  ...
  describe('caminho global (sem schoolId)', () => {
    it('retorna { id } não-vazio ao criar ausência sem schoolId', async () => {
      const result = await callCreate({ teacherId: TEACHER_ID, slots: [] }) as Record<string, unknown>
    ...
    it('grava status: "open" inicial', async () => {
      await callCreate({ teacherId: TEACHER_ID, slots: [] })
      const savedAbsence = mockDocSet.mock.calls[0][0] as Record<string, unknown>
  ...
  describe('normalização de campos do slot', () => {
    it('grava substituteId: null em cada slot', async () => {
      ...
      const savedAbsence = mockDocSet.mock.calls[0][0] as Record<string, unknown>
      const slots = savedAbsence.slots as Record<string, unknown>[]
    ...
    it('campos ausentes no slot de entrada são gravados como null', async () => {
      ...
      const savedAbsence = mockDocSet.mock.calls[0][0] as Record<string, unknown>
      const slot = (savedAbsence.slots as Record<string, unknown>[])[0]
  ...
  describe('propagação de erro de autorização', () => {
    it('propaga erro de verifyCoordinatorOrAdmin sem gravar', async () => {
      const auth = jest.requireMock('../auth') as { verifyCoordinatorOrAdmin: jest.Mock }
      auth.verifyCoordinatorOrAdmin.mockRejectedValueOnce(
...
describe('updateAbsence', () => {
  ...
  describe('ausência não encontrada', () => {
    ...
    it('lança not-found quando doc.exists é false', async () => {
      await expect(
        callUpdate({ absenceId: ABSENCE_ID, slots: [] })
      ).rejects.toMatchObject({ code: 'not-found' })
    })
  ...
  describe('payload de update', () => {
    it('chama doc.update com { slots, substituteId, status }', async () => {
      ...
      expect(mockDocUpdate).toHaveBeenCalledWith(
        expect.objectContaining({ slots, substituteId: null, status: 'open' })
      )
    })
...
describe('deleteAbsence', () => {
  ...
  describe('propagação de erro de autorização', () => {
    it('propaga erro de verifyCoordinatorOrAdmin sem deletar', async () => {
      ...
      await expect(
        callDelete({ absenceId: ABSENCE_ID })
      ).rejects.toMatchObject({ code: 'permission-denied' })
    ...
    it('propaga erro de autorização com schoolId fornecido sem deletar', async () => {
      ...
      await expect(
        callDelete({ absenceId: ABSENCE_ID, schoolId: SCHOOL_ID })
